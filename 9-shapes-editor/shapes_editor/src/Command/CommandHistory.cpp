...
void CommandHistory::AddAndExecute(std::unique_ptr<Command>&& command)
{
	command->Execute();
...
void CommandHistory::Redo()
{
	if (!CanRedo())
	{
		throw std::logic_error("Unable to redo command: no commands available for redo");
	}
...
void CommandHistory::Undo()
{
	if (!CanUndo())
	{
		throw std::logic_error("Unable to undo command: no commands available for undo");
	}
